...
def calculate_proximity_score(symbol_file: str, current_file: str | None) -> float:
...
class ContextInjector:
    ...
    def calculate_relevance_score(
        ...
        if (
            ...
            and hasattr(symbol, "embedding")
            and symbol.embedding is not None
        ):
            try:
    ...
    def _calculate_semantic_similarity(
        self, query_emb: np.ndarray, symbol_emb: np.ndarray
    ) -> float:
    ...
    def calculate_available_budget(self, existing_context_tokens: int = 0) -> int:
        ...
        available = self.model_context_window - existing_context_tokens - protected_tokens

        return max(0, available)

    def select_top_k(self, symbols: list[dict], k: int | None = None) -> list[dict]:
        ...
        sorted_symbols = sorted(
            symbols, key=lambda s: getattr(s, "relevance_score", 0.0), reverse=True
        )

        # Enforce diversity
        return self.history.enforce_diversity(sorted_symbols, limit=self.diversity_limit)[:k]
