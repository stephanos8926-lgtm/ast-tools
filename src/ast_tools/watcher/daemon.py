...
class WatcherDaemon:
    ...
    def __init__(
        ...
        self.include_extensions = include_extensions or [
            ...
            '.json', '.yaml', '.yml', '.sh', '.bash', '.md'
        ]
        self.exclude_patterns = exclude_patterns or [
            '__pycache__', '.git', 'node_modules', 'venv', '.venv',
            '*.log', '*.tmp', '.DS_Store', '.egg-info', 'dist', 'build'
        ]
        self.queue = IndexQueue(debounce_ms=debounce_ms)
        self.observer: Optional[Observer] = None
        self.handler: Optional[AstToolsHandler] = None
        self.running = False
    
    def start(self) -> None:
        """Start watching all paths."""
        self.handler = AstToolsHandler(
            self.queue,
    ...
    def run(self, reindex_callback: Optional[Callable[[str], None]] = None) -> None:
...
def reindex_file(file_path: str) -> dict:
    ...
    try:
        ...
        # Extract symbols
        symbols, edges = extract_symbols(result.tree, file_path)
