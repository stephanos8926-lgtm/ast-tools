    ...
    try:
        ...
        if TREND_FILE.exists():
            ...
            if isinstance(history, list) and len(history) > 0:
                ...
                TREND_FILE.write_text(json.dumps([{"score": score, "timestamp": datetime.now().isoformat()}]))
        else:
            TREND_FILE.write_text(json.dumps([{"score": score, "timestamp": datetime.now().isoformat()}]))
    except Exception as e:
...
def cli_doctor(args: dict | list | None = None) -> str:
    ...
    if trend.get("previous") is not None:
        ...
        delta_str = f" (+{delta})" if delta and delta > 0 else f" ({delta})" if delta and delta < 0 else " (no change)"
    ...
    if verbose:
        ...
        for c in result.get("checks", []):
            status_char = "✅" if c["status"] == "ok" else "⚠️" if c["status"] == "warn" else "❌" if c["status"] == "fail" else "⏭️"
