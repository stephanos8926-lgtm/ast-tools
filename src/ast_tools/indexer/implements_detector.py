...
class ImplementsDetector(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef) -> None:
        ...
        is_interface_subclass = any(self._is_interface_base(base_name) for base_name in base_names)
    ...
    def _is_interface_base(self, base_name: str) -> bool:
        ...
        return base_name in self._interface_methods
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        if self._current_class:
            ...
            if is_abstract:
                ...
                logger.debug(f"Registered abstract method: {self._current_class}.{func_name}")
            ...
            for base_name in bases:
                if base_name in self._interface_methods:
                    ...
                    for method_name in interface_methods:
                        ...
                        edges.append(edge)
                        logger.debug(f"DETECTED: {class_name} implements {target_name}")

        return edges


def find_implements_relationships(tree: ast.AST, file_path: str) -> list[Edge]:
