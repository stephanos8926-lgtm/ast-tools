...
class ParseResult:
    ...
    __slots__ = ("content_hash", "error", "success", "tree")
...
def parse_file(file_path: Path) -> ParseResult:
    ...
    try:
        ...
        try:
            ...
            return ParseResult(success=False, error="Permission denied")
        except Exception as e:
            logger.warning(f"Read error for {file_path}: {e}")
            return ParseResult(success=False, error=f"Read error: {e}")
        ...
        if not content.strip():
            ...
            return ParseResult(success=True, tree=empty_module, content_hash=content_hash)

        # Parse AST
        tree = ast.parse(content, filename=str(file_path))
        return ParseResult(success=True, tree=tree, content_hash=content_hash)
        ...
        return ParseResult(success=False, error=f"Unexpected error: {e}")
