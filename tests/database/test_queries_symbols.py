...
class TestSymbolQueries:
    ...
    def test_insert_and_get_symbol(self, db_conn: sqlite3.Connection):
        ...
        symbol = Symbol(
            id="file1.py:foo", name="foo", qualified_name="foo", 
            kind=SymbolKind.FUNCTION, file_path="file1.py", 
            start_line=1, end_line=3, signature="def foo():", 
            docstring="docs", is_public=True, content_hash="hash1"
        )
    ...
    def test_insert_symbols_batch(self, db_conn: sqlite3.Connection):
        ...
        symbols = [
            Symbol(id=f"f.py:s{i}", name=f"s{i}", qualified_name=f"s{i}", 
                   kind=SymbolKind.FUNCTION, file_path="f.py", 
                   start_line=i, end_line=i+1, signature="", docstring="", 
                   is_public=True, content_hash="h")
            for i in range(5)
        ]
    ...
    def test_search_symbols_fts(self, db_conn: sqlite3.Connection):
        ...
        symbols = [
            Symbol(id="f1.py:a", name="func_a", qualified_name="func_a", 
                   kind=SymbolKind.FUNCTION, file_path="f1.py", 
                   start_line=1, end_line=2, signature="", docstring="Search me", 
                   is_public=True, content_hash="h1"),
            Symbol(id="f2.py:b", name="class_b", qualified_name="class_b", 
                   kind=SymbolKind.CLASS, file_path="f2.py", 
                   start_line=1, end_line=5, signature="", docstring="Ignore me", 
                   is_public=True, content_hash="h2"),
        ]
        insert_symbols_batch(db_conn, symbols)
    ...
    def test_find_symbol_definition(self, db_conn: sqlite3.Connection):
        ...
        symbol = Symbol(
            id="f1.py:test", name="test", qualified_name="test", 
            kind=SymbolKind.FUNCTION, file_path="f1.py", 
            start_line=1, end_line=2, signature="", docstring="", 
            is_public=True, content_hash="h"
        )
        insert_symbol(db_conn, symbol)
    ...
    def test_list_symbols_by_file(self, db_conn: sqlite3.Connection):
        ...
        symbols = [
            Symbol(id="f1.py:s1", name="s1", qualified_name="s1", 
                   kind=SymbolKind.FUNCTION, file_path="f1.py", 
                   start_line=1, end_line=2, signature="", docstring="", 
                   is_public=True, content_hash="h"),
            Symbol(id="f1.py:s2", name="s2", qualified_name="s2", 
                   kind=SymbolKind.FUNCTION, file_path="f1.py", 
                   start_line=3, end_line=4, signature="", docstring="", 
                   is_public=True, content_hash="h"),
            Symbol(id="f2.py:s3", name="s3", qualified_name="s3", 
                   kind=SymbolKind.FUNCTION, file_path="f2.py", 
                   start_line=1, end_line=2, signature="", docstring="", 
                   is_public=True, content_hash="h"),
        ]
        insert_symbols_batch(db_conn, symbols)
