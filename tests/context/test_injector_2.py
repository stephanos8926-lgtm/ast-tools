...
class TestBudgetManagement:
    ...
    def test_calculate_available_budget(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            injector = ContextInjector(
                Path(tmpdir), model_context_window=32000, protect_last_n_messages=2
            )

            available = injector.calculate_available_budget(existing_context_tokens=5000)
    ...
    def test_select_top_k_within_budget(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            injector = ContextInjector(Path(tmpdir), max_context_symbols=10)
...
class TestDiversityEnforcement:
    ...
    def test_diversity_limit_enforced(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            injector = ContextInjector(Path(tmpdir), diversity_limit=3)
